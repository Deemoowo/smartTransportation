...
public class VectorStoreService {
    ...
    private static final String COLLECTION_NAME = "transportation_knowledge";
    ...
    private List<SearchResult> parseSearchResults(SearchResults searchResults) {
        ...
        for (int i = 0; i < searchResults.getResults().getTopK(); i++) {
            ...
            result.setScore(1.0f); // 实际应从结果中解析
            result.setContent("搜索结果内容"); // 实际应从结果中解析
            result.setMetadata("元数据"); // 实际应从结果中解析
            result.setSource("数据源"); // 实际应从结果中解析
            results.add(result);
        }
